...
<p align="center">
  <em><a href="https://github.com/patty-io/pi-patty-bg-tasks">patty-io/pi-patty-bg-tasks</a>의 fork — 같은 백그라운드 작업 툴킷이되, 입력하면 중단하는 기본 동작을 걷어냈습니다. bash 명령이 도는 중에 타이핑하면 명령을 중단하는 대신 스티어링으로 큐에 쌓입니다. 이는 Pi의 기본 스티어링과도, Claude Code의 실제 동작과도 일치합니다.</em>
</p>
...
## 명령이 도는 중의 스티어링 (fork 변경)

이 fork는 상류 patty-io/pi-patty-bg-tasks 익스텐션의 **입력하면 중단하는 기본 동작을 걷어냅니다.**

- **상류 동작 (이 fork는 이렇게 하지 않습니다):** 백그라운드로 넘길 수 있는 포그라운드 명령이 도는 중에 메시지를 입력하면, Pi가 그 텍스트를 스티어링으로 큐에 넣기 **전에** 익스텐션이 먼저 끼어듭니다 — 명령은 백그라운드로 미끄러져 들어가고, 현재 에이전트 턴은 중단되며, 에이전트가 idle이 되는 순간 메시지가 새 사용자 턴으로 다시 올라옵니다.
- **이 fork:** `bash` 명령이 도는 중에 입력해도 명령을 **중단하지 않습니다.** Pi의 **기본 스티어링**이 적용됩니다 — 메시지는 큐에 쌓였다가 다음 턴 경계에서 전달되고, 명령은 계속 돕니다. 잃는 건 없습니다 (출력은 잡의 로그에 그대로 캡처됩니다).

나머지는 상류와 동일합니다: 120초 자동 백그라운드, Ctrl+B 수동 백그라운드, `jobs` 매니저, `monitor` 도구, 정체 감지, `agent_bg` 전부 그대로입니다. 오직 입력 가로채기 / 입력 시 중단 기본값만 제거되었습니다 — `src/input.ts`는 이제 가로채기를 거절하고 항상 Pi에게 넘깁니다.

**왜:** 오래 걸리는 명령은 계속 돌아야 하고, 큐에 쌓인 스티어링은 제 차례를 기다리면 됩니다. 타이핑을 시작하자마자 명령이 헐리는 건 곤란합니다.

이 지점에서 이 fork는 Claude Code 동등성을 버린 게 아니라 오히려 **맞춘** 것입니다. Claude Code는 입력했다고 명령을 중단하지 않습니다 — **Claude Code 2.1.231**(2026-08-13)에서 pty로 실제 대화형 세션을 구동해 측정한 결과: 포그라운드 bash 명령이 도는 중에 메시지를 제출해도 명령은 전혀 건드려지지 않았고(어떤 시그널도 없이 48초 관측 창 내내 full rate로 동작), 툴 콜은 열린 채 남았으며, UI에는 *"Press up to edit queued messages"*가 표시되었습니다. Claude Code는 큐에 넣습니다. 상류의 입력 시 중단은 상류 자신의 설계 선택이었지 Claude Code 동등성이 아니며, 위 문단은 예전에 그 반대로 적혀 있었습니다.

Claude Code는 **Esc**를 누르면 포그라운드 명령을 실제로 죽이고(약 80ms 안에 프로세스 그룹으로 SIGTERM, 약 1초 뒤 SIGKILL), **Ctrl+B**나 자동 백그라운드 타임아웃에서는 실제로 백그라운드로 넘깁니다 — 이 fork가 이미 맞추고 있는 동작입니다. 오직 타이핑 하나만 예전 설명이 거꾸로였습니다.

**적용 범위:** 도구가 도는 중에 입력한 모든 메시지에 해당합니다. 익스텐션이 감싸지 않는 다른 오래 걸리는 도구는 원래부터 Pi 기본 스티어링(큐에 쌓였다가 다음 턴 경계에서 전달)으로 처리됩니다.
...
- **취소에 대한 Claude Code 동등성 (CC 소스로 검증).** **Esc**를 누르면 실행 중인 포그라운드 명령이 종료되고(의도적 취소), **Ctrl+B**·자동 백그라운드 타임아웃은 대신 백그라운드로 옮깁니다 — CC의 `user-cancel` vs `interrupt` 동작 그대로입니다. 긴 작업은 취소를 무시하는 게 아니라 타임아웃 시 자동 백그라운드 + `run_in_background`로 보호합니다.
  - *사후 정정:* 이 항목은 원래 "새 메시지 입력"도 명령을 백그라운드로 옮긴다고 적고 있었습니다. 그렇지 않습니다. **Claude Code 2.1.231** 기준(2026-08-13 측정) 타이핑은 포그라운드 명령을 그대로 계속 돌게 두고 메시지만 큐에 넣습니다. Esc·Ctrl+B·타임아웃 항목은 영향이 없습니다. "명령이 도는 중의 스티어링" 절을 참고하세요.

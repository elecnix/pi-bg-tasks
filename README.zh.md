...
<p align="center">
  <em><a href="https://github.com/patty-io/pi-patty-bg-tasks">patty-io/pi-patty-bg-tasks</a> 的 fork —— 同一套后台任务工具,但去掉了「输入即中断」的默认行为:在 bash 命令运行期间打字会排进转向队列,而不是中断命令。这同时符合 Pi 的原生转向,也符合 Claude Code 的真实行为。</em>
</p>
...
本 fork **去掉了**上游 patty-io/pi-patty-bg-tasks 扩展的「输入即中断」默认行为。

- **上游行为(本 fork 不这么做):** 前台正跑着一条可转后台的命令时,你打了一条消息 —— 扩展会在 Pi 把它排进转向队列**之前**就先一步介入:命令滑入后台,当前代理轮次被中断,代理一空闲,你的消息就作为一个全新的用户轮次重新注入。
- **本 fork:** 在 `bash` 命令运行期间输入**不会**中断它。走 Pi 的**原生转向** —— 消息排队,在下一个轮次边界投递,命令继续跑。什么都不会丢(输出照常写进作业日志)。

其余部分与上游完全一致:120 秒自动转后台、Ctrl+B 手动转后台、`jobs` 管理器、`monitor` 工具、卡顿检测、`agent_bg` 全都照旧。只有输入拦截 / 输入即中断这一个默认行为被移除 —— `src/input.ts` 现在一律不拦截,直接交回 Pi。

**为什么:** 长命令应当继续跑,让排队的转向等到它该到的轮次,而不是你一开始打字就被拆掉。

这一点上本 fork 是**补上**了与 Claude Code 的一致性,而不是丢掉它。Claude Code 并不会因为你输入就中断命令 —— 在 **Claude Code 2.1.231**(2026-08-13)上用 pty 驱动真实交互式会话实测:前台 bash 命令运行期间提交消息,命令完全没被动过(没有任何信号,整个 48 秒观察窗口内保持满速运行),工具调用仍然挂着,界面显示 *"Press up to edit queued messages"*。Claude Code 是排队。上游的「输入即中断」是上游自己的设计选择,并不是与 Claude Code 对齐;这一段以前的说法是反的。

Claude Code **确实**会在按 **Esc** 时杀掉前台命令(约 80ms 内向进程组发 SIGTERM,约 1 秒后 SIGKILL),也确实会在 **Ctrl+B** 或自动转后台超时时把它转到后台 —— 这些本 fork 早就一致。只有「打字输入」这一项,旧描述搞反了。

**适用范围:** 这对每一条在工具运行期间输入的消息都成立。扩展没有包裹的其他长任务工具,本来就走 Pi 原生的转向机制(排队,在下一个轮次边界投递)。
...
- **取消行为对齐 Claude Code(已对照 CC 源码验证)。** 按 **Esc** 会杀掉正在运行的前台命令(一次有意的取消),而 **Ctrl+B** 或自动后台超时则改为把它移到后台——正是 CC 的 `user-cancel` 与 `interrupt` 之分。长任务靠超时自动后台 + `run_in_background` 来保护,而不是忽略取消。
  - *事后更正:* 这一条原本还把「输入新消息」列为会让命令转后台。并不会。截至 **Claude Code 2.1.231**(2026-08-13 实测),打字会让前台命令原地继续运行,消息则被排队。Esc、Ctrl+B 和超时三条不受影响。详见「运行中命令的转向」。
